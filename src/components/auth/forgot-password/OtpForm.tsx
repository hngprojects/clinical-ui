...
export function OtpForm({
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center flex-1 max-w-[521px] mx-auto animate-fadeIn">
      ...
      <h1 className="text-center text-[#191C21] text-2xl md:text-3xl font-bold font-['Inter'] leading-tight mb-2">
        Verify Your Identity
      </h1>

      <div className="text-center mb-6 md:mb-8 flex flex-col items-center justify-center">
        <span className="text-[#424752] text-sm md:text-base font-medium font-['Inter'] leading-relaxed block">
          We&apos;ve sent a 6-digit verification code to
        </span>
        <span className="text-[#191C21] text-sm md:text-base font-medium font-['Inter'] leading-relaxed block mt-0.5">
          {maskEmail(email)}
        </span>
      </div>

      <form onSubmit={onSubmit} className="w-full flex flex-col items-center gap-4 md:gap-6">
        <div className="flex justify-between items-center gap-2 sm:gap-4 w-full max-w-[384px]">
          {otp.map((data, index) => (
            <input
              key={index}
              type="text"
              ...
              className="w-10 h-12 md:w-12 md:h-14 px-2 py-2 text-center text-[#191C21] text-xl md:text-2xl font-medium font-['Inter'] bg-[#F8FAFC] rounded-lg border border-slate-300 focus:border-[#1565C0] focus:ring-2 focus:ring-[#1565C0]/10 outline-none transition-all disabled:opacity-50"
            />
          ))}
        </div>
