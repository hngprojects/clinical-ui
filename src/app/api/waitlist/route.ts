...
export async function POST(request: Request) {
  try {
    ...
    const WAITLIST_API_URL = process.env.NEXT_PUBLIC_WAITLIST_API_URL || 'https://api.staging.clinsight.hng14.com/api/v1/waitlist';
      ...
      if (!response.ok) {
        ...
        if (contentType && contentType.includes('application/json')) {
          try {
            ...
            errorMessage = err.detail || err.message || err.error || JSON.stringify(err) || 'Backend error';
          } catch {
